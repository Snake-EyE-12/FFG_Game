...
public class FingerGun : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (aiming)
        {
            ...
            if (Physics.Raycast(leftRay, out RaycastHit leftHit, 1000f, wallMask))
            {
                leftAimPos = leftHit.point;
            }
            else
            {
                leftAimPos = leftDir * 1000; 
            }
            if (Physics.Raycast(rightRay, out RaycastHit rightHit, 1000f, wallMask))
            {
                rightAimPos = rightHit.point;
            }
            else
            {
                rightAimPos = rightDir * 1000;
            }
    ...
    public void Shoot()
    {
        if (aiming && gunLoaded)
        {
            ...
            if (Physics.Raycast(shootRay, out RaycastHit hit, Mathf.Infinity, shootLayerMask))
            {
